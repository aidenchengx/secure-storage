...
from swift.obj.crypter import Crypto
from swift.obj.keyprocess import keygenerate,keyfetch
from swift.obj.DB import Conn
import string
...
class ObjectController(BaseStorageServer):
    ...
    def container_update(self, op, account, container, obj, request,
        ...
        """
	self.logger.info(_("PLEASE BE REASON!"))
	...
	etag2=md5()
		...
		encrlevel=request.headers.get('X-Encryption-Level')
		if encrlevel is None:
		    try:
		    ...
		    #Get the IV from proxy node, create one if not exists
		    iv=request.headers.get('X-Encryption-IV')
	 	    if not iv:
		         iv=os.urandom(16)
		    info1={'type':encrlevel,
          		   }
		    string1=md5()	
		    if encrlevel =='account':
			string1.update(account)
		    elif encrlevel =='container':
			string1.update(container+account)
		    else:
			string1.update(account+container+obj)#the token must be limit to 32-bytes 
			#should consider if we use md5 to encrypt
		    info1['token']=string1.hexdigest()
		    key=b''#initialized a bytes str		    
    		    keyge=keygenerate(info1)#will retrieve an existing key or create one
		    key=keyge.keytext 
                    try: 
                    	for chunk in iter(timeout_reader, ''):
			    start_time = time.time()
   		            ck=Crypto(key,iv)
		            en=ck.create_encryption_ctxt()
		            ct=en.update(chunk)+en.finalize()
                            upload_size = writer.write(ct)                        
                            if start_time > upload_expiration:
                                self.logger.increment('PUT.timeouts')
                                return HTTPRequestTimeout(request=request)
                            etag.update(chunk)  #etag saves the plaintext imfo
			    etag2.update(ct)    #etag2 saves the ciphertext info
			    elapsed_time += time.time() - start_time   
		    #if encrlevel == 'object':
		...
		if encrlevel is None:
		    metadata = {
                    ...
                    'ETag': etag,
                    'Content-Length': str(upload_size),
	   	    'Encryption-level':info1['type'],
		    'Encryption-token':info1['token'],
		    'IV':iv,
                }
                ...
                for header_key in headers_to_copy:
                    if header_key in request.headers:
                        header_caps = header_key.title()
                        metadata[header_caps] = request.headers[header_key]
		metadata['ETag']=etag2		#in the storage node it's storing a cipher md5
                writer.put(metadata)
    ...
    def GET(self, request):
		...
		if 'IV' in metadata:
		    ...
		    info2={'type':'',
          	'token':''}
		    info2['type']=metadata['Encryption-level']
		    info2['token']=metadata['Encryption-token']
		    key=b''
		    keyfe=keyfetch(info2)
    		    key=keyfe.keytext#retrieve the key	
		    length=65536#a chunk size
		    tt=response.body
		    bb=b''
		    q=[tt[x:x+length] for x in range(0,len(tt),length)]
		    for chunk in q:
               	    	ck=Crypto(key,iv)
                    	en=ck.create_encryption_ctxt()
                    	ct=en.update(chunk)+en.finalize()
		    ...
		    response.body=bb		
		    etag2=md5()		    
		    etag2.update(bb)		#etag2 is the plaintext md5
		    etag2=etag2.hexdigest()
		    response.etag=etag2		#sends the plaintext md5
		else:
		    pass
