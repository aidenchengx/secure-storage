...
class BaseObjectController(Controller):
    ...
    def PUT(self, req):
        ...
        partition, nodes = obj_ring.get_nodes(
            self.account_name, self.container_name, self.object_name)
	self.app.logger.info('PUT')
	self.app.logger.info(req.headers.keys())
        # pass the policy index to storage nodes via req header
        req.headers['X-Backend-Storage-Policy-Index'] = policy_index
        next_part_power = getattr(obj_ring, 'next_part_power', None)
        if next_part_power:
	...
	encrlevel=req.headers.get('Encrypt-Level')
	if encrlevel is None or encrlevel=='none':
		pass
	else:	#proxy node create a IV
		req.headers['X-Encryption-Level']=encrlevel
		req.headers['X-Encryption-IV']=os.urandom(16)
	encrlevel=req.headers.get('X-Encryption-Level')
...
class ReplicatedObjectController(BaseObjectController):
	...
	#self.app.logger.info('obj.py')
	#self.app.logger.info(req.getheaders())
...
class MIMEPutter(Putter):
	...
	#Create a IV
	headers['X-Encryption-IV']=os.urandom(16)	
